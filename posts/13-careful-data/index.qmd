...
Update: Thanks to [u/Sufficient_Meet6836](https://www.reddit.com/user/Sufficient_Meet6836/) from this [thread](https://www.reddit.com/r/rstats/comments/1q2vagq/comment/nxkp6ov/?utm_source=share&utm_medium=web3x&utm_name=web3xcss&utm_term=1&utm_content=share_button) for pointing it out, and Hadley Wickham respectfully recommends `.data[[colname]]`, rather than `.data$colname`.

Here's why:

1.  `.data$colname` will throws an error once used in a closure:  

    ``` r
    new_col = function(data, var1, var2) {
        data |> 
            dplyr::mutate(new = .data$var1 / .data$var2) |> 
            head(5)
    }
    
    mtcars |> new_col(mpg, wt)
    ```

2.  `.data[[colname]]` with `ensym()` is less painful and accepts both bare columns and stringed columns: 

    ``` r
    new_col = function(data, var1, var2) {
        box::use(rlang[ensym, as_string], dplyr[mutate])
        var1 = as_string(ensym(var1))
        var2 = as_string(ensym(var2))
        
        data |> 
            mutate(new = .data[[var1]] / .data[[var2]]) |> 
            head(5)
    }
...
read_csv(
    "data.csv",
    col_types = cols(
        .default = col_character(), # This force everything as text first 
    )
)
```
